...
function App() {
  ...
  // Estados para almacenar la informacion del Backend
  ...
  // Datos derivados del auth (se calculan siempre)
  const usuarioNombre =
  ...
  /* --- ZONA DE INTEGRACION CON BACKEND --- */
  // AHORA el hook esta al nivel superior (correcto)
  useEffect(() => {
    // Solo hacemos fetch si esta autenticado y hay token
    ...
    const endpoint = esDocente
      ? "/api/profesor/mis-alumnos"
      : "/api/alumno/mis-calificaciones";
    ...
    fetch(urlCompleta, {
      ...
      .catch((err) => {
        console.error(err);
        // Aqui podrias setear un estado de error para mostrar en UI
      })
      .finally(() => setCargandoDatos(false));
  }, [auth.isAuthenticated, token, esDocente]);

  // 1. Estado de Carga (Autenticacion)
  if (auth.isLoading) {
    return (
  ...
  if (auth.error) {
    return (
      <div className="text-red-400 p-8 bg-gray-800 h-screen">
        Error de conexion: {auth.error.message}
      </div>
    );
  }

  // 3. Renderizado Principal (Solo si esta autenticado)
      ...
      <div className="min-h-screen bg-gray-800 flex flex-col">
        ...
        <main className="flex-grow p-6 md:p-10">
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div
              className={`bg-gray-700 rounded-lg shadow-2xl overflow-hidden border-t-4 ${
                esDocente ? "border-purple-500" : "border-blue-500"
              }`}
            >
              <div className="p-6 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    {esDocente
                      ? `Gestionando materia: ${
                          datosProfesor.materiaImpartida || "Cargando..."
                        }`
                      : "Consulta tus calificaciones parciales"}
                  </p>
                </div>
                {/* Icono decorativo segun rol */}
                    ...
                    <tbody className="text-gray-200 divide-y divide-gray-600">
                      {datosProfesor.alumnos.length === 0 ? (
                        <tr>
                          ...
                          >
                            No hay alumnos registrados o no se pudo cargar la
                            lista.
                          </td>
                        </tr>
                      ) : (
                      ...
                      {datosAlumno.length === 0 ? (
                        <tr>
                          <td
                            ...
                            className="p-8 text-center text-gray-400"
                          >
                            No tienes materias asignadas o calificaciones
                            registradas.
                          </td>
                        </tr>
                      ) : (
